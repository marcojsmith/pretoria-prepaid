...
function prepareChartData(allMonthlyStats: { month: string; units: number }[]) {
  const data = [];
  const now = new Date();
...
function calculateMaxAvg(rollingData: { avgDaily: number }[]) {
  const max = Math.max(...rollingData.map((d) => d.avgDaily));
  return max === 0 ? MIN_CHART_VALUE : max * MAX_CHART_SCALE_FACTOR;
}
...
function ChartBar({
  ...
  data: { monthKey: string; month: string | undefined; avgDaily: number };
  maxAvg: number;
  index: number;
}) {
...
export function AverageDailyUsageChart(): JSX.Element {
  ...
  const rollingData = useMemo(() => prepareChartData(allMonthlyStats), [allMonthlyStats]);

  const maxAvg = useMemo(() => calculateMaxAvg(rollingData), [rollingData]);
      ...
      <CardContent>
        <div className="flex h-[180px] justify-between gap-1 pt-4 sm:gap-2">
          {rollingData.map((data, index) => (
            <ChartBar key={data.monthKey} data={data} maxAvg={maxAvg} index={index} />
          ))}
        </div>
